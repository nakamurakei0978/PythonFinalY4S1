...
export const useProductStore = defineStore('product', () => {
    ...
    async function postProduct(name, price, qty, category_id, imageFile){
        ...
        try{
            const res = await axios.post('http://localhost:5000/post_product',formData,{
    ...
    async function searchProducts(text){
        ...
        try{
            const res = await axios.get('http://localhost:5000/search_products',{
                params:{
                    limit: limitRows.value,
                    page: page.value,
                    searchName: text 
    ...
    async function putProduct(id,name,qty,price,category_id,imageFile){
        ...
        formData.append('image', imageFile)
        try{
            const res = await axios.put('http://localhost:5000/put_product',formData,{
                headers: { 'Content-Type': 'multipart/form-data' }
            })
    ...
    async function deleteProduct(id){
        try{
            const res = await axios.delete(`http://localhost:5000/delete_product/${id}`)
