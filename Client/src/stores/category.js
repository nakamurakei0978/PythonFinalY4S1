...
export const useCategoryStore = defineStore('category', () => {
    ...
    async function postCategory(name, imageFile) {
        ...
        const res = await axios({
            method: 'POST',
            url: 'http://127.0.0.1:5000/post_category',
            data: formData,
    ...
    async function getCategories(limit) {
        ...
        if (limit === 'All') {
            try {
                const res = await axios.get('http://127.0.0.1:5000/get_categories')
                totalPage.value = 1
                page.value = 1
            ...
            try {
                const res = await axios.get(`http://127.0.0.1:5000/get_categories?limit=${limit}&page=${page.value}`)
    ...
    async function putCategory(id, name, imageFile) {
        ...
        const res = await axios({
            method: 'PUT',
            url: `http://127.0.0.1:5000/put_category/${id}`,
            data: formData,
            headers: { 'Content-Type': 'multipart/form-data' },
        }).then((res) => {
    ...
    async function deleteCategory(id) {
        const res = await axios({
            method: 'DELETE',
            url: `http://127.0.0.1:5000/delete_category/${id}`
        }).then(async (res) => {
            await getCategories(limitRows.value)
            return res.data.message
        }).catch((err) => {
    ...
    async function searchCategories(search){
        try {
            const res = await axios.get('http://127.0.0.1:5000/search_categories', {
